...
import AdminDesk from "../components/admin_desk/admin_desk";
...
const Home = () => {
  ...
  const [isModButtonActive,setModButtonActive] = useState(false);

  const [all_desks, setAllDesks] = useState([]);
  const [all_admin_desks, setAllAdminDesks] = useState([]);
  ...
  const adjustSelectGroupStyle = () =>{
    ...
    selectGroup.style.backgroundColor = 'red'
    selectGroup.style.top = 0;
    selectGroup.style.left = 0;


    let cursor = document.getElementsByClassName('selectable-selectbox')[0];
  ...
  function loadDesks (positions, building_city,building_floor){
    ...
    for (const position of positions) {
        ...
        th_all_admin_desk.push(<AdminDesk pos_x={position[0]} pos_y = {position[1]}  deskStatus={"disponible"} tableId={i} building_city={building_city} building_floor={building_floor} 
        date={date} start_time={startTime} end_time={endTime} />)
        i++
    }
